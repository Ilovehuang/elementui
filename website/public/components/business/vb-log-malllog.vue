...
<div>
   <vx-view-datatable :options="Options" :tabledata="List" :rowdata="rowdata" :delrow.sync="Delrow">
      ...
      <vd-bd-grid url="/MallUpdateService.html" :query="query" :result.sync="List" :trigger.sync="trigger"></vd-bd-grid>
</div>
</template>

<script>
Vue.component("vb-log-malllog", {
   template: template,
   props: [
      "query", /// 查询数据条件
      "trigger" /// 表格更新操作后的标记
