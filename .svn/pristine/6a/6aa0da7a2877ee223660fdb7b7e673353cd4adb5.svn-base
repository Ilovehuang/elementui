<template>
   <vx-view-datatable :options="Options" :tabledata="List" :rowdata.sync="RowData" :modifyrow.sync="ModifyForm" :deleterow.sync="DeleteTrigger">
      <slot></slot>
      <vd-company-holiday operate="load" :trigger.sync="LoadTrigger" :query="Query" :result.sync="List"></vd-company-holiday>
      <vd-company-holiday operate="save" method="del" :trigger.sync="DeleteTrigger" :query="RowData" :result.sync="DeleteResult"></vd-company-holiday>
   </vx-view-datatable>
</template>
<script>
Vue.component("vm-grid-holiday", {
   data() {
      return {
         Options: {
            paging: true,
            ...
            columns: [
               ...
               {
                  title: LoaderDict.Business.T_Holiday.HolidayType, // "节日类型",
                  width: 80,
                  className: "text-center",
                  data: "HolidayType"
               },
