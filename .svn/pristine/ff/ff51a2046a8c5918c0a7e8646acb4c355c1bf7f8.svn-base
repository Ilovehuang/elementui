<template>
   <vx-view-datatable :options="GridOptions" :checked="Checked" :tabledata="List" :rowdata.sync="RowData" :modifyrow.sync="ModifyForm" :deleterow.sync="DeleteTrigger">
      <slot></slot>
      <vd-company-devicezone operate="load" :autoload="autoload" :method="method" :trigger.sync="LoadTrigger" :query="Query" :result.sync="List"></vd-company-devicezone>
...
<script>
Vue.component("vm-subset-devicezone", {
   data(){
      return {
         method:"",
         autoload:"",
   ...
   created() {
      ...
      this.$Bus.$on("DeviceZone.Grid", function(data) {
         me.Query = data;
   ...
   methods: {
      ...
      modifyzone(){
         this.$Bus.$emit("DeviceZoneModify.Form", this.RowData);
      }
   },
